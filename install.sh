...
# @@File             :  install.sh
# @@Description      :  Install configurations for gtk-2.0
...
__run_pre_install() {
  ...
  \find /lib* /usr/lib* /usr/local/lib* -iname '*libgtk*2*.so*' -type f 2>/dev/null | grep -F '.' | grep -q '^' && SYSTEM_PACKAGES="true"
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
